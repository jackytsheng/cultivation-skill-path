...
- AI 生成可下载 `.json` 文件后，点击“选择道法文件”直接导入
- AI 只能在聊天里输出 JSON 时，把完整 JSON 粘贴到文本框，再点“导入道法文本”
...
导入时会先选择一条预设境界路线。应用会按照所选路线给境界命名：
...
7. 下载文件后，在应用里点“选择道法文件”导入
8. 如果 AI 不能生成文件，才把完整 JSON 粘贴回文本框，并点击“导入道法文本”
...
道法引会要求 AI 自动补全剩下的境界、十层、每层任务和任务次数。它会明确要求 AI 不要一条条分批生成 JSON；能创建附件时直接生成一个完整、可下载、UTF-8 编码的 `.json` 文件。
